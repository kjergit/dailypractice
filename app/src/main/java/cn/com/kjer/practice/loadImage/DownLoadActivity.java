...
import cn.com.kjer.practice.utils.DiskLruCache;

/**
 * download images and load the pic  by lru
 * copy by internet  id= guolin
...
public class DownLoadActivity extends BaseActivity {
    ...
    protected void onFindViews() {
        super.onFindViews();
        downLoadGridView = (GridView) findViewById(R.id.download_gv);
    }

    @Override
    protected void onSetData() {
        super.onSetData();
        adapterDownLoad = new DownLoadAdapter(this, 0, imageThumbUrls, downLoadGridView);
        downLoadGridView.setAdapter(adapterDownLoad);
        downLoadGridView.getViewTreeObserver().addOnGlobalLayoutListener(
                new ViewTreeObserver.OnGlobalLayoutListener() {

                    @Override
                    public void onGlobalLayout() {
                        final int numColumns = (int) Math.floor(downLoadGridView.getWidth() /
                                (mImageThumbSize + mImageThumbSpacing));
                        if (numColumns > 0) {
                            int columnWidth = (downLoadGridView.getWidth() /
                                    numColumns) - mImageThumbSpacing;
                            adapterDownLoad.setItemHeight(columnWidth);
                            downLoadGridView.getViewTreeObserver().removeGlobalOnLayoutListener(this);
                        }
                    }
                });
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        // 退出程序时结束所有的下载任务
        adapterDownLoad.cancelAllTasks();
    }
